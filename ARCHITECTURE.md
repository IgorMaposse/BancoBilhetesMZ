# BancoBilhetes — Documento de Arquitetura

Exercício técnico — Sénior Software Engineer, DSTI Moza Banco.
...
| Serviço-a-serviço | `client_id`/`client_secret` em headers | Mesmo padrão usado nas APIs internas reais da Fidelidade (`fidelidademundial.com`), com o qual o candidato tem experiência direta; em produção seria substituído por OAuth2 Client Credentials |
...
- **Premissa assumida:** dado o prazo do exercício, a chamada ao
  ...
  recomendar-se-ia um padrão *outbox* + fila de mensagens (ex.: Kafka/RabbitMQ)
...
- Comunicação `ticketing-platform` → `payment-platform` autenticada por
  `client_id`/`client_secret` em headers — o mesmo padrão usado nas
  integrações internas reais da Fidelidade, escolhido deliberadamente pela
  familiaridade e por ser proporcional ao âmbito do exercício. Em produção
...
| ticketing-platform (Swagger) | http://localhost:8080/swagger-ui.html |
| payment-platform (Swagger) | http://localhost:8081/swagger-ui.html |

Para correr cada módulo individualmente sem Docker (perfil `dev`, H2 em
memória), ver o `README.md` de cada módulo.
